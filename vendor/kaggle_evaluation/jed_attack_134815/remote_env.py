...
class AttackSessionCancelled(BaseException):
    """Signal that the gateway has retired an expired attack session."""
...
class RemoteEnv:
    ...
    def _receive_response(self) -> Any:
        result = self._response_queue.get()
        if isinstance(result, AttackSessionCancelled):
            raise result
        return result
    ...
    def snapshot(self) -> Any:
        ...
        return self._receive_response()  # Returns snapshot_id (string handle)

    def restore(self, snapshot: Any) -> None:
        """Restore from a snapshot handle ID."""
        self._request_queue.put({"op": "restore", "snapshot_id": snapshot})
        self._receive_response()  # Wait for ack
...
class AttackSession:
    ...
    def __init__(self, attack_cls, budget_s: float, seed: int):
        ...
        self.error: Exception | AttackSessionCancelled | None = None
    ...
    def _run_attack(self) -> None:
        ...
        except AttackSessionCancelled as err:
            self.error = err
