...
# Six phases fit within the hosted 15-hour limit while leaving time for model
# setup, response serialization, cancellation, unloading, and score output.
DEFAULT_BUDGET_S = 8750.0
ATTACK_ENV_OP_GRACE_S = 5.0
GATEWAY_RESPONSE_TIMEOUT_BUFFER_S = 175.0
...
ATTACK_BUDGET_EXCEEDED_ERROR = (
    "Attack time budget exceeded while waiting for attack env operation"
)
ENV_SELECTION = "gym"
ATTACK_SEED = 123
...
class ModelEvaluationTimedOut(RuntimeError):
    """Raised when a gateway-owned evaluation phase exceeds its hard deadline."""
...
def _run_until_deadline(
    ...
    *,
    ...
    """Run a phase on a daemon thread without trusting it to terminate itself."""
...
class JEDAttackGateway(kaggle_evaluation.core.templates.Gateway):
    ...
    def generate_data_batches(self) -> Generator[Any, None, None]:
    ...
    def _cancel_attack_session(self, model_name: str) -> None:
        """Retire a timed-out participant before using the next model."""
        client = self.client
        previous_timeout = client.endpoint_deadline_seconds
        client.endpoint_deadline_seconds = min(
            previous_timeout,
            int(2 * ATTACK_ENV_OP_GRACE_S),
        )
        try:
            response = _require_attack_server_response(
                self.predict({"cmd": "cancel"})
            )
        finally:
            client.endpoint_deadline_seconds = previous_timeout

        if response.get("status") != "cancelled" or response.get("retired") is not True:
            raise GatewayRuntimeError(
                GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
                f"Timed-out attack session for model {model_name} could not be safely retired",
            )
    ...
    def _run_attack_for_model(
        ...
        snapshots: dict[str, Any] = {}
        snapshot_state_tokens: dict[str, int] = {}
        # Keep one replayable checkpoint per trajectory, not one per message.
        completed_candidates: list[dict[str, Any]] = []
        active_candidate_index: int | None = None
        active_candidate_state_token: int | None = None
        # Tokens identify actual env states even when message histories match.
        current_state_token = 0
        next_state_token = 1

        # Start the attack session in the inference server.
        print(f"[ATTACK][{model_name}] Sending init command to inference server...")
        generation_deadline_s = time.monotonic() + budget_s
        response = _require_attack_server_response(
            self.predict({
                "cmd": "init",
                "budget_s": budget_s,
        ...
        while response.get("status") == "env_op":
            if time.monotonic() >= generation_deadline_s:
            ...
            if op == "reset":
                ...
                active_candidate_state_token = None
                current_state_token = next_state_token
                next_state_token += 1
            elif op == "snapshot" and isinstance(result, str):
                snapshot_state_tokens[result] = current_state_token
            elif op == "restore" and isinstance(result, Mapping) and result.get("ack") is True:
                snapshot_id = response.get("snapshot_id")
                if isinstance(snapshot_id, str):
                    restored_state_token = snapshot_state_tokens.get(snapshot_id)
                    if restored_state_token is not None:
                        if (
                            restored_state_token != current_state_token
                            and restored_state_token != active_candidate_state_token
                        ):
                            active_candidate_index = None
                            active_candidate_state_token = None
                        current_state_token = restored_state_token
            elif op == "interact":
                current_state_token = next_state_token
                next_state_token += 1
                if (
                    active_candidate_index is not None
                    or len(completed_candidates) < MAX_REPLAY_FINDINGS
                ):
                    ...
                    trace_user_messages = trace.get("user_messages", [])
                    if len(trace_user_messages) <= MAX_REPLAY_MESSAGES_PER_FINDING:
                        candidate_index = (
                            active_candidate_index
                            if active_candidate_index is not None
                            else len(completed_candidates)
                        )
                        try:
                            user_messages = _validate_serialized_candidate_messages(
                                {"user_messages": trace_user_messages},
                                candidate_index,
                            )
                        except (TypeError, ValueError):
                            # Exploratory interactions need not satisfy the submission contract.
                            pass
                        else:
                            if time.monotonic() >= generation_deadline_s:
                                generation_timed_out = True
                                break
                            candidate = {"user_messages": list(user_messages)}
                            if active_candidate_index is None:
                                active_candidate_index = len(completed_candidates)
                                completed_candidates.append(candidate)
                            else:
                                completed_candidates[active_candidate_index] = candidate
                            active_candidate_state_token = current_state_token
        ...
        if status == "timeout":
            if (
                response.get("error_type") != ATTACK_TIMEOUT_ERROR_TYPE
                or response.get("error") != ATTACK_BUDGET_EXCEEDED_ERROR
                or time.monotonic() < generation_deadline_s
            ):
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.GATEWAY_RAISED_EXCEPTION,
                    "Attack server returned an invalid attack-timeout response",
                )
            generation_timed_out = True

        if generation_timed_out:
            self._cancel_attack_session(model_name)
            print(
                f"[ATTACK][{model_name}] Attack phase timed out; preserving "
                f"{len(completed_candidates)} completed, gateway-observed candidates"
            )
    ...
    def _replay_and_score(
        ...
        for idx, candidate_data in enumerate(candidates[:MAX_REPLAY_FINDINGS]):
            ...
            replay_candidates.append((idx, user_messages))

        resolved_budget_s = DEFAULT_BUDGET_S if budget_s is None else budget_s
        replay_deadline_s = time.monotonic() + resolved_budget_s
        agent_factory = self._make_agent_factory(model_name)
        validated_findings: list[ValidatedAttackFinding] = []
        no_predicates = 0
        ...
        for idx, user_messages in replay_candidates:
            ...
            try:
                ...
                finding: ValidatedAttackFinding = {
                    "seed": trace["seed"],
    ...
    def get_all_predictions(self) -> tuple[Any, list[Any]]:
        ...
        for model_idx, model_name in enumerate(MODEL_NAMES):
            ...
            try:
                ...
                generation_deadline_s = (
                    time.monotonic()
                    + DEFAULT_BUDGET_S
                    + ATTACK_ENV_OP_GRACE_S
                    + GATEWAY_RESPONSE_TIMEOUT_BUFFER_S
                )
                ...
                for guardrail_label, guardrail_factory in guardrail_configs.items():
                    ...
                    model_results["guardrail_scores"][guardrail_label] = {
                        ...
                        "candidates_replayed": result.get(
                            "candidates_replayed", len(result["findings"])
                        ),
                    }
            ...
            except ModelEvaluationTimedOut as err:
                raise GatewayRuntimeError(
                    GatewayRuntimeErrorType.INVALID_SUBMISSION,
                    str(err),
                ) from err
                ...
                # Timed-out workers or unretired participant sessions may still
                # own the model, so unload only after every phase has completed.
    ...
    def write_submission(self, predictions: Any, row_ids: Any) -> None:
