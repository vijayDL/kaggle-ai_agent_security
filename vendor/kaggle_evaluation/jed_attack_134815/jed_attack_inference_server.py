...
ATTACK_TIMEOUT_ERROR_TYPE = jed_attack_gateway.ATTACK_TIMEOUT_ERROR_TYPE
ATTACK_SESSION_CANCEL_JOIN_S = jed_attack_gateway.ATTACK_ENV_OP_GRACE_S
...
def _budget_exceeded_response() -> dict[str, str]:
    return {
        "status": "timeout",
        "error": jed_attack_gateway.ATTACK_BUDGET_EXCEEDED_ERROR,
...
def predict(request: dict[str, Any]) -> dict[str, Any]:
    ...
    if cmd == "cancel":
        session = _session
        if session is None:
            return {"status": "cancelled", "retired": True}

        if not session.is_done():
            session.send_env_result(
                renv.AttackSessionCancelled("Attack session cancelled")
            )
            if session.thread is not None:
                session.thread.join(timeout=ATTACK_SESSION_CANCEL_JOIN_S)

        retired = session.is_done()
        if retired:
            _session = None
        return {"status": "cancelled", "retired": retired}

    if cmd == "init":
        if _session is not None and not _session.is_done():
            return _exception_response(
                RuntimeError("Previous attack session has not been retired")
            )
