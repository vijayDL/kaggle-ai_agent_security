# v18-probehops (2026-08-09, see STATE.md): PUBLIC-chasing bet, 1-hop fill probing isolated
# (PROBE_HOPS 0->1, REPLAY_COST_COEF 1.0->1.5). Otherwise byte-identical to attack_v17_edge.py
# (88.920, current best validated). SDK 3.1.2 assumptions re-verified live 2026-08-09 against
# the real aicomp_sdk download, not from memory -- see vendor/aicomp_sdk-3.1.2/.
...
PROBE_HOPS = 1                # 2026-08-09 v18 PUBLIC BET: probe at 1 hop instead of the full
                              # 8-hop cap -- the scored exfil event fires at hop-0 for every
                              # primitive in this chassis (single-post TEMPLATE/FRAME_TEMPLATE,
                              # SLOW_MULTIPOST_N stays at its default 1 in this variant so
                              # every candidate is genuinely single-hop), so probing shorter
                              # should skip the scoring-irrelevant wrap-up generation for more
                              # fill throughput. Self-correcting either way: _fired() only
                              # keeps candidates that actually fired live, so a bad interaction
                              # with the current model server just yields fewer keeps, never an
                              # invalid submission.
REPLAY_COST_COEF = 1.5        # kept > 1.0 (not the docstring's bare "8-hop/1-hop ratio") since
                              # this exact lever is unmeasured on live Kaggle T4 hardware post-
                              # reset; charges each kept candidate 1.5x its measured 1-hop cost
                              # against the replay-safe budget so a worse-than-expected true
                              # 8-hop replay cost still fits, at the cost of some headroom.
