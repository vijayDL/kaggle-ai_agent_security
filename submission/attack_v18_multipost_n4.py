# v18-multipost-n4 (2026-08-09, see STATE.md): PUBLIC-chasing bet, gpt_oss Harmony-forge
# multipost isolated (SLOW_MULTIPOST_N 1->4). Otherwise byte-identical to attack_v17_edge.py
...
SLOW_MULTIPOST_N = 4          # 2026-08-09 v18 PUBLIC BET: Harmony-forge multipost on the
                              # gpt_oss (slow) row only -- verified untouched by the still-
                              # unfixed Gemma {{ }}-brace parser bug (SDK 3.1.2 checked live
                              # 2026-08-09: gemma4_agent.py regex is still strict single-brace
                              # native grammar, no {{ }} normalization present despite the Aug
                              # host announcement -- this lever never touches that path since
                              # SLOW_MULTIPOST_N only fires via _forge_plan_msg on the split-
                              # routed slow (gpt_oss) row). N=4 matches the previously-measured
                              # safe rung (4.0 posts/cand on the real GGUF, pre-reset).
