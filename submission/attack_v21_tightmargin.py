"""v21-tightmargin (2026-08-20): v17-edge chassis, ONE knob changed.

REPLAY_SAFE_FRAC 0.99 -> 0.995. Per v17-edge's own header comment: "0.99 * 9000
= 8910s -> ~90s margin under the 9000 attack.py timeout". At 0.995 that margin
becomes ~45s. Worth re-testing now specifically because the risk calculus
changed on 2026-08-08: a replay-phase timeout now PRESERVES accumulated score
instead of voiding it (REPLAY_BUDGET_MULT=5.0 already assumes this), so the
only thing REPLAY_SAFE_FRAC still guards is the attack.py wall_deadline itself
-- and overrunning THAT still hard-terminates. This has never been re-tuned
since the Aug-8 eval change; today's public-hunting slot #2, since the
Gemma-doubled-brace re-check (redteam/run_gemma_doublebrace_check.py,
2026-08-20) came back still-broken with no new lever to spend the slot on
instead. Real, if small, risk: too-tight a margin risks a whole-row 0 on
wall_deadline overrun -- this is a modest single-rung tightening (47s
"safe" -> ~45s cushion at REPLAY_SAFE_FRAC=0.995), not an aggressive one, and
below the CANARY value's own MARGIN_S=47.0 in absolute terms as a sanity
check. If this doesn't beat the same-day v17-edge control (also submitted
today), the margin isn't the current bottleneck and shouldn't be pushed
further.
...
REPLAY_SAFE_FRAC = 0.995      # v21-tightmargin: 0.99 -> 0.995, ~90s -> ~45s wall cushion
