...
SLOW_MULTIPOST_N = 3          # 2026-08-08 FORCED-MULTIPOST test: N=3 endpoints/candidate,
                              # LOW N to stay well under the 8-hop budget (n7's cliff was the
                              # hop limit). Forge multipost fires N EXFIL/candidate; local shows
                              # 2.8x raw/sec but real hop-cost is untested (prior runs were
                              # latency-gated). This is the clean board test of that lever.
...
class AttackAlgorithm(AttackAlgorithmBase):
    ...
    def _fill(
        ...
        while len(cands) < cap:
            ...
            if burst_k == 1:
                # FORCED MULTIPOST: always use the forge-multipost message (gate removed).
                # The prior split-latency `slow_row` gate meant multipost often never fired;
                # here every candidate posts to N endpoints so the lever is actually pulled.
                if slow_multipost_n > 1:
