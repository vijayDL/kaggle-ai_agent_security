# v18-no-ok (2026-08-13, see STATE.md): PUBLIC-chasing bet, drop the "Then answer OK only."
# wrap-up instruction entirely from BOTH the classification-phase TEMPLATE and the slow-row
# FRAME_TEMPLATE override. Motivated by the same replay-cost reasoning as v18-gemma-forge (see
# STATE.md): if the wrap-up instruction is what's driving hop-1's generation, removing it might
# reduce per-candidate replay cost -- OR it might backfire if the model rambles more without a
# terse directive (genuinely uncertain, that's why this is isolated from every other variant).
# The tool-call instruction itself (the proven-reliable verbose form) is UNCHANGED -- only the
# trailing wrap-up sentence is removed.
