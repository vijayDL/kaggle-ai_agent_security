"""v22-margin999 (2026-08-21): v17-edge chassis, ONE knob pushed further than
v21-tightmargin dared.

v21-tightmargin moved REPLAY_SAFE_FRAC 0.99 -> 0.995 (~90s -> ~45s wall cushion)
and landed at 86.985 vs the same-day v17-edge control's 86.580 -- a +0.4 delta
indistinguishable from the day's own run-to-run noise (confirmed spread that day:
86.580-90.385 on UNCHANGED code). A single-rung nudge was too small to separate
signal from noise either way.

This variant pushes REPLAY_SAFE_FRAC to 0.999 (~9s wall cushion, vs today's
~90s->~45s->~9s ladder) -- a decisively bigger step, not another marginal one.
Rationale for going this far: the 2026-08-08 eval change made a REPLAY-phase
timeout partial-credit-preserving (REPLAY_BUDGET_MULT=5.0 already assumes this),
so REPLAY_SAFE_FRAC's only remaining job is guarding the attack.py WALL_DEADLINE
itself, which still hard-terminates (voids the whole row) on overrun -- that is
the one real risk here, and it's asymmetric (small upside per extra candidate,
total-row-zero downside if the ~9s cushion is consumed by a single slow
candidate). Genuinely informative either way: if 0.999 also lands flat/noisy,
that confirms sizing-knob tuning is fully exhausted on this axis (matches
IDEA.md's 2026-08-21 conclusion that the P1-throughput axis is closed) and we
stop touching REPLAY_SAFE_FRAC for good; if it either wins clearly or voids a
row, we learn the real edge of the safe range instead of guessing at it.
...
REPLAY_SAFE_FRAC = 0.999      # v22-margin999: 0.99 -> 0.999, ~90s -> ~9s wall cushion (decisive step)
