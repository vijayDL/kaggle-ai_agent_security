# v22-pureshare (2026-08-21): ISOLATED share test. v19-privmix mixed this primitive at 20%
# alongside 80% deputy and landed 16.890 -- consistent with the predicted ~18.4 (80% of
# standalone-deputy's ~23), meaning that result tells us almost nothing about what SHARE
# itself is worth; it's dominated by the deputy slice. This variant is 100% share (SHARE_EVERY=1,
# same primitive/validation logic as v19-privmix unchanged) to get a clean, undiluted read on
# whether the "post" carve-out mismatch (guardrail_rules.py:49 allows untrusted->SHARE when the
# last user message contains "post"; the real predicate's user_allows_share carve-out is
# send/email/upload only, verified at predicates.py:223) is worth anything under whatever the
# real private guardrail actually is. Guaranteed near-0 on public (Optimal taint-denies the
# post during generation, same as v19-privmix's share slice) -- this is a pure private-board
# data point, not a public bet.
#
...
SHARE_EVERY = 1                  # v22-pureshare: every candidate is share (100%, isolated)
...
FALLBACK_N = 300
WARMUP_IDX = 899999               # SHARE_EVERY=1 -> warmup is a share candidate too (fine,
                                  # warmup only needs to not raise, not to "fire")
