# v18-multipost-n7 (2026-08-10, see STATE.md): PUBLIC-chasing bet, third dose-response point
# alongside v18-multipost-n4 (79.740, -10.3%) and v18-multipost-n6 (90.385, +1.6%) -- N=7
# instead of N=4/N=6/N=8. Otherwise byte-identical to attack_v17_edge.py (88.920, current best
# validated). SDK 3.1.2 assumptions re-verified live 2026-08-09 against the real aicomp_sdk
# download, not from memory -- see vendor/aicomp_sdk-3.1.2/.
...
SLOW_MULTIPOST_N = 7          # 2026-08-10 v18 PUBLIC BET: third dose-response point,
                              # deliberately chosen at exactly 1 hop of slack under the
                              # fixed 8-hop replay cap (8-7=1), vs N=8's zero slack and
                              # N=6's 2 hops of slack. Motivated by the v18-probehops result
                              # (63.535, a real -28.5% regression): tight hop budgets can
                              # silently collapse fire rate rather than just trim waste, so
                              # N=7 tests whether that same cliff is already visible before
                              # the hard N=8 ceiling, or whether the N=6 gain (+1.6% vs
                              # baseline, unconfirmed/1 data point) keeps holding. gpt_oss
                              # row only; see attack_v18_multipost_n4.py for the gemma-brace-
                              # bug non-interaction note.
